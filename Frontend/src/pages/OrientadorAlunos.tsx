import { useQuery } from '@tanstack/react-query';
import { FileText, Users } from 'lucide-react';
import AppLayout from '@/components/AppLayout';
import LoginPage from '@/components/LoginPage';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { useAuth } from '@/contexts/AuthContext';
import { getApos, getStudents, queryKeys } from '@/lib/api';
import type { APORecord, AlunoResumo } from '@/lib/mock-data';

export default function OrientadorAlunos() {
  const { user } = useAuth();
  const { data: apos = [] } = useQuery({ queryKey: queryKeys.apos, queryFn: getApos, enabled: !!user });
  const { data: students = [] } = useQuery({ queryKey: queryKeys.students, queryFn: getStudents, enabled: !!user });

  if (!user) {
    return <LoginPage />;
  }

  const orientadorApos = getAposDoOrientador(apos, user.id);
  const alunos = getAlunosDoOrientador(students, orientadorApos, user.id);

  return (
    <AppLayout>
      <div className="max-w-5xl space-y-6">
        <div>
          <h1 className="text-2xl font-display font-bold">Alunos Orientados</h1>
          <p className="font-body text-sm text-muted-foreground">Acompanhe seus orientados e o status das APOs</p>
        </div>

        <Card className="shadow-card">
          <CardHeader>
            <CardTitle className="text-lg font-display">Lista de Alunos</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {alunos.length === 0 ? (
              <div className="p-8 text-center font-body text-muted-foreground">
                <Users className="mx-auto mb-2 h-8 w-8 text-accent" />
                Nenhum aluno orientado encontrado.
              </div>
            ) : (
              alunos.map((aluno) => (
                <AlunoOrientadoCard key={aluno.id} aluno={aluno} apos={orientadorApos.filter((apo) => apo.alunoId === aluno.id)} />
              ))
            )}
          </CardContent>
        </Card>
      </div>
    </AppLayout>
  );
}

function AlunoOrientadoCard({ aluno, apos }: { aluno: AlunoResumo; apos: APORecord[] }) {
  return (
    <div className="rounded-lg border border-border/70 p-4">
      <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-display font-semibold">{aluno.nome}</p>
          <p className="font-body text-xs text-muted-foreground">{aluno.pontosAcumulados}/12 pontos acumulados</p>
        </div>
        <Badge variant="outline">{apos.length} APO(s)</Badge>
      </div>

      <div className="mt-4 space-y-2">
        {apos.length === 0 ? (
          <p className="rounded-md bg-secondary/40 px-3 py-2 font-body text-sm text-muted-foreground">Nenhuma APO enviada por este aluno.</p>
        ) : (
          apos.map((apo) => (
            <div key={apo.id} className="flex flex-col gap-2 rounded-md bg-secondary/40 px-3 py-2 md:flex-row md:items-center md:justify-between">
              <div className="flex min-w-0 items-center gap-2">
                <FileText className="h-4 w-4 shrink-0 text-muted-foreground" />
                <span className="truncate text-sm font-display font-medium">{apo.titulo}</span>
              </div>
              <Badge className={apo.status === 'em_avaliacao_orientador' ? 'bg-info/15 text-info border-info/20' : 'bg-success text-success-foreground'}>
                {apo.status === 'em_avaliacao_orientador' ? 'Pendente' : 'Avaliada'}
              </Badge>
            </div>
          ))
        )}
      </div>
    </div>
  );
}

function getAposDoOrientador(apos: APORecord[], orientadorId: string) {
  const vinculadas = apos.filter((entry) => entry.orientadorId === orientadorId);
  return vinculadas.length > 0 ? vinculadas : apos;
}

function getAlunosDoOrientador(students: AlunoResumo[], apos: APORecord[], orientadorId: string) {
  const vinculados = students.filter((entry) => entry.orientadorId === orientadorId);
  if (vinculados.length > 0) {
    return vinculados;
  }

  const alunoIdsComApo = new Set(apos.map((entry) => entry.alunoId));
  return students.filter((entry) => alunoIdsComApo.has(entry.id));
}
